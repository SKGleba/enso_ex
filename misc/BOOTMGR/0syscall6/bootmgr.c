...
// patch rvk_check & patch sm_load to allow custom kprxauthsm
static const unsigned char sk_rvk_patch_nmp[] = {
...
// if magic is good, overwrite kprxauthsm in memory and resume process
static const unsigned char sm_load_patch_nmp[] = {
...
static int load_ussm(int *ctx, uint32_t sm_sz_arg) {
	...
	if (read_f2b(smsz, sm_block_addr, "os0:zss_ussm.self") > 0)
		return 1;
...
static void cmep_run(uint32_t sm_sz_arg) {
	...
	*(uint16_t *)CALL_KA_NOP_A = 0xBF00; // patch call_kprxauthsm_func to not modify 3rd arg
	...
	if (read_f2b(rvksz, (payload_block_addr + 0x300), "os0:zss_ka.elf") == 0) { // add custom kprx_auth_sm if present
		...
		*(uint32_t *)(payload_block_addr + 0x208) = ((uint32_t)rvksz - 0x1000); // size of .elf \ header
	}
