    ...
    if (fill_pixels <= 0) {
        for (int i = y; i < y + height; i++)
            for (int j = x; j < x + width; j++)
                view_draw_pixel(paper->view_idx, j, i, color);
    } else {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if ((i < fill_pixels) || (i >= height - fill_pixels) || (j < fill_pixels) || (j >= width - fill_pixels)) {
                    view_draw_pixel(paper->view_idx, x + j, y + i, color);
                }
            }
        }
    }
