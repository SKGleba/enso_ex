...
static void s3_rwudi_sqh(enum FMGR_ENTRY_TYPES entype, char *path, char *entry, enum VIEW_ASSIGNS *next_uview) {
    view_switch(VIEW_DEFAULT);  // switch to the log view
    LOG("s3_rwudi_sqh(entype=%d, path=%s, entry=%s)\n", entype, path, entry);
                ...
                if (ret >= 0) {
                    ...
                    if (ret >= 0) {
                        ...
                        if (ret >= 0) {
                            ...
                            if (ret >= 0)
                                alllog("UDIs successfully dumped to %s\n", fpath);
                            else
                                alllog("Failed to write UDIs to file: 0x%08X\n", ret);
                        } else
                            alllog("Failed to read OpenPSID leaf 1: 0x%08X\n", ret);
                    } else
                        alllog("Failed to read OpenPSID leaf 0: 0x%08X\n", ret);
                } else
                    alllog("Failed to read CID leaf: 0x%08X\n", ret);
            } else {
                                ...
                                if (ret >= 0) {
                                    ...
                                    if (ret >= 0) {
                                        ...
                                        if (ret >= 0) {
                                            ...
                                            if (ret >= 0) {
                                                alllog("Wrote OpenPSID leaf 0\n");
                                                ret = idstorage_rw_leaf(1, S3_LEAF_OPSID_1, leaf2);
                                                if (ret >= 0) {
                                                    alllog("Wrote OpenPSID leaf 1\n");
                                                    alllog("UDIs written successfully\n");
                                                } else
                                                    alllog("Failed to write OpenPSID leaf 1: 0x%08X\n", ret);
                                            } else
                                                alllog("Failed to write OpenPSID leaf 0: 0x%08X\n", ret);
                                        } else
                                            alllog("Failed to read OpenPSID leaf 1: 0x%08X\n", ret);
                                    } else
                                        alllog("Failed to read OpenPSID leaf 0: 0x%08X\n", ret);
                                } else
                                    alllog("Failed to write CID leaf: 0x%08X\n", ret);
                            } else
                                alllog("Failed to read CID leaf: 0x%08X\n", ret);
                        } else
                            alllog("Operation cancelled by user.\n");
                    } else
                        alllog("File %s has unexpected size: %d\n", fpath, f_br);
                    my_free(fdata);
                } else
                    alllog("Failed to read file %s\n", fpath);
            }
            my_free(buf);
        } else
            alllog("Failed to allocate memory for buffer\n");
        idstorage_stop();
    } else
        alllog("Failed to initialize ID storage: 0x%08X\n", ret);
        ...
        switch (selection) {
            ...
            case 2: // Dump/Flash Unique Device IDs
                if (fmgr_square_handler(0, 0, NULL) != s3_rwudi_sqh) {
                    alllog("Use [] in the file manager to select source file or destination directory for UDIs.\n");
                    alllog("Press X again to cancel.\n");
                    fmgr_square_handler(1, BITN(FMGR_ENTRY_TYPE_DIR) | BITN(FMGR_ENTRY_TYPE_FILE), s3_rwudi_sqh);
                } else {
                    alllog("Canceling the UDI operation.\n");
                    fmgr_square_handler(1, 0, NULL);
                }
                return MENU_RET_CONTINUE;
            case 3: // Reboot the system
                alllog("Rebooting the system...\n");
                syscon_reset_device(SYSCON_RESET_TYPE_COLD_RESET, 0);
                return MENU_RET_CONTINUE;
            case 4: // Power off the device
                alllog("Powering off the device...\n");
