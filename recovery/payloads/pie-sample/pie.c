...
static int init_logview(bool init) { // initialize display and exr temp view for visual log
    if (init) {
        if (r_eex_params->boot_mode == BOOTSTRAP_MODE_LIB) {
            r_ILOG("Enabling display & views\n");
            if (r_bmx_displaymgr(DISPLAYMGR_NSTATE_ON, DISPLAYMGR_OPT_INCLUDE_FB) < 0) {
                r_ELOG("Failed to initialize display!\n");
                return -1;
            }
            if (r_view_init() < 0) {
                r_ELOG("Failed to initialize views!\n");
                return -1;
            }
        } else {
            DACR_OFF(
                rxp_prev_log_targets = r_log_outputs;
            );
        }
        r_view_switch(VIEW_TEMP);
        r_ILOG("Initializing the default view..\n");
        ...
        r_log_outputs |= LOG_TARGET_LOGPAPER;
    } else {
        r_log_outputs = rxp_prev_log_targets;
        r_default_paper->view_idx = VIEW_DEFAULT;
        r_view_switch(VIEW_DEFAULT);
        if (r_eex_params->boot_mode == BOOTSTRAP_MODE_LIB) {
            r_ILOG("Disabling display & views\n");
...
__attribute__((section(".text.start"))) int start(void) {
    ...
    r_ILOG("Recovery blob initialized successfully\n");
    main();
