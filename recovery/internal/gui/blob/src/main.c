...
void (*eex_init_os0)(int mbr_off) = NULL;
...
static int init(struct sysroot_buffer *sysroot, void (*init_os0)(int mbr_off)) {
	...
	cdram_enable();
	syscon_init();
	LOG("cdram & syscon initialized!\n");

	if (sysroot_model_is_dolce())
        bmx_display_init(DISPLAY_TYPE_HDMI);
    else if (sysroot_model_is_vita2k())
        bmx_display_init(DISPLAY_TYPE_LCD);
    else
        bmx_display_init(DISPLAY_TYPE_OLED);
    LOG("Display initialized!\n");

	eex_init_os0 = init_os0;
...
int main(int stage) {
    ...
    scrprintf_align(CENTER, "enso_ex recovery menu\nby skgleba\n");
    ...
    scrprintf("Welcome to the enso_ex recovery menu!\n");
    scrprintf("Use arrow keys to navigate, X to select.\n");
    ...
    switch (stage) {
		...
		case 3:
			...
			break;
        default:
            scrprintf("ERROR: Unknown stage %d\n", stage);
            break;
    }
        ...
        if (BMX_CTRL_BUTTON_HELD(current_menu->prs_buttons, CTRL_DOWN) && (current_menu->selection < current_menu->entry_count - 1))
            ...
            switch (ret) {
                ...
                case MENU_RET_FINISH_DEINIT:
                    ...
                    deinit((struct sysroot_buffer *)0x51167528);
                    break;
                default:
                    scrprintf("ERROR: Unknown menu return value %d\n", ret);
                    break;
            }
        }
    }
