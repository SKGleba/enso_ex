...
int bmx_display_init(enum display_type type) {
    display_init(type);
    ...
    bmx_fb_uid = sceKernelAllocMemBlock("e2x_recovr_vram", 0x60208006, ds_config->pitch * ds_config->height * 4, &opt);
    if (bmx_fb_uid < 0) {
    ...
    LOG("Framebuffer allocated at %p\n", fb_base);
    return 0;
}
