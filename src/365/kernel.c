...
int write_config_sa() {
	...
	printf("write_config_sa\n");

	ksceIoMkdir("sa0:eex", 0777); // make directory if it does not exist
	...
	ret = fd = ksceIoOpen("sa0:eex/boot_config.txt", SCE_O_WRONLY | SCE_O_CREAT | SCE_O_TRUNC, 0777);
	if (ret < 0) {
		printf("failed to open sa0:eex/boot_config.txt for write: 0x%08x\n", ret);
		ret = -1;
		goto cleanup;
	}
	...
	const char *patch1 = 
		...
		"# enso_ex kernel boot configuration script.\n\n"
		"- include\tux0:eex/boot_config.txt\n"
		"- include\tur0:tai/boot_config.txt\n\n"
		"# if it fails - proceed with this one\n";
	len = strlen(patch1);
	if ((ret = ksceIoWrite(fd, patch1, len)) != len) {
		printf("failed to write config 1st part: wrote 0x%08x expected 0x%08x\n", ret, len);
		ret = -1;
	...
	// write 3rd part: patch: load taihen and henkaku
	const char *patch2 = "\n- load\tsa0:eex/taihen.skprx\n- load\tsa0:eex/henkaku.skprx\n";
	len = strlen(patch2);
	if ((ret = ksceIoWrite(fd, patch2, len)) != len) {
		printf("failed to write config 3rd part: wrote 0x%08x expected 0x%08x\n", ret, len);
		ret = -1;
...
int k_ensoWriteConfig() {
	int retu = 0, rets = 0, ret = 0;
	int state = 0;

	ENTER_SYSCALL(state);
	retu = run_on_thread(write_config_ur);
	rets = run_on_thread(write_config_sa);
	EXIT_SYSCALL(state);
...
int write_blocks(void) {
	...
	ret = fat_fd = ksceIoOpen("ur0:temp/slim.bin", SCE_O_RDONLY, 0);
	if (ret < 0) {
...
int module_stop() {
	...
	printf("umounting sa0..\n");
	ksceIoUmount(0xB00, 0, 0, 0);
	ksceIoUmount(0xB00, 1, 0, 0);
	ksceIoMount(0xB00, NULL, 2, 0, 0, 0);
