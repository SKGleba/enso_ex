...
static int get_file(char* file_path, void* buf, uint32_t read_size, uint32_t offset) {
	printf("[BOOTMGR] get_file [0x%X] @ %s\n", read_size, file_path);
...
static int load_sm(int* ctx, char* argpath, unsigned int smsz) {
	...
	if (get_file(argpath, sm_block_addr, smsz, 0))
		return -1;
	...
	if (smtool_invoke(0, paddr_pairs_paddr, paddr_list.ret_count, NULL, &smcomm_ctx, ctx) < 0) // load the SM
		return 3;
	*(uint16_t*)0x51016cf2 = 0x2000; // remove the loadsm call from load_kprauthsm (since ussm is already loaded)
	*(uint16_t*)0x51016cf4 = 0x2000; //
	clean_dcache((void*)0x51016cf0, 0x10);
	flush_icache();
	if (smtool_load_ka() < 0) // enable cry2arm, TZ magic, SM start
		return 6;
	*(uint16_t*)0x51016cf2 = 0xf7ff; // undo load_kprxauthsm patches
	*(uint16_t*)0x51016cf4 = 0xf915; //
	clean_dcache((void*)0x51016cf0, 0x10);
	flush_icache();
...
static void cmep_run() {
	...
	*(uint16_t*)0x51016bec = 0xBF00; // patch call_kprxauthsm_func to not modify 3rd arg
	clean_dcache((void*)0x51016be0, 0x10);
	flush_icache();
	...
	*(uint16_t*)0x51016bec = 0x6073; // undo the 3rd arg patch
	clean_dcache((void*)0x51016be0, 0x10);
	flush_icache();
